...
bath20= Dataset("/home/mitya/models/NorROMS/Apps/Common/Grid/arctic4km_grd_bath20.nc")
bath4 = Dataset("/home/mitya/models/NorROMS/Apps/Common/Grid/arctic4km_grd.nc")
...
ho=bath20.variables["h"][:]
print ho.shape, "ho"
#sys.exit()

hi = bath20.variables["h"][:]
...
z_ri = z_coord.spec_vert_grid(Ni,hi,b[0][b[1].index("zeta")][0,:],Tclinei,theta_bi,theta_si,Vtransformi,Vstretchingi,[b[0][b[1].index("zeta")][0,:].shape[0],b[0][b[1].index("zeta")][0,:].shape[1]])
print z_ri.shape
print np.amax(z_ri), np.amin(z_ri)

z_ro = z_coord.spec_vert_grid(No,ho,b[0][b[1].index("zeta")][0,:],Tclineo,theta_bo,theta_so,Vtransformo,Vstretchingo,[b[0][b[1].index("zeta")][0,:].shape[0],b[0][b[1].index("zeta")][0,:].shape[1]])
print z_ro.shape
...
for field in args.field:
    print field
    if field == "u":
        b[0][b[1].index(field)][0,:] = np.transpose(vert.vert_int(np.transpose(b[0][b[1].index(field)][0,:], (1,2,0)),z_ri[:,:mask_u.shape[1]],z_ro[:,:mask_u.shape[1]],undef,mask_u,mask_u,[mask_u.shape[1],mask_u.shape[0],Ni,No]),(2,0,1))[:]
    elif field == "v":
         b[0][b[1].index(field)][0,:]  =np.transpose( vert.vert_int(np.transpose(b[0][b[1].index(field)][0,:], (1,2,0)),z_ri[:mask_v.shape[0],:],z_ro[:mask_v.shape[0],:],undef,mask_v,mask_v,[mask_v.shape[1],mask_v.shape[0],Ni,No]),(2,0,1))[:]
    else:
        b[0][b[1].index(field)][0,:] = np.transpose(vert.vert_int(np.transpose(b[0][b[1].index(field)][0,:], (1,2,0)),z_ri,z_ro,undef,mask_rho,mask_rho,[b[0][b[1].index("zeta")][0,:].shape[1],b[0][b[1].index("zeta")][0,:].shape[0],Ni,No]), (2,0,1))[:]    
