...
bath4 = Dataset("/home/mitya/models/NorROMS/Apps/Common/Grid/arctic20km_grd.nc")
f = Dataset(args.inf)
...
#print b[0][b[1].index(args.field)][0,:,10,200]
    ...
    for field in args.field:
        ...
        if field == "u":
            ...
            print np.transpose(vert.vert_int(np.transpose(b[0][b[1].index(field)][record,:], (1,2,0)),z_ri,z_ro,undef,mask_rho,mask_rho,[mask_rho.shape[1],mask_rho.shape[0],Ni,No]), (2,0,1)).shape
            b[0][b[1].index(field)][record,:] = np.transpose(vert.vert_int(np.transpose(b[0][b[1].index(field)][record,:], (1,2,0)),z_ri,z_ro,undef,mask_rho,mask_rho,[mask_rho.shape[1],mask_rho.shape[0],Ni,No]), (2,0,1))[:]
