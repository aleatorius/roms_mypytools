...
string =str("""ncap2 -O -s 'xl=""")+str(int((ranges["xi_rho"][1]-ranges["xi_rho"][0])*4000))+"""' constants_sliced.nc -o constants_sliced.nc"""
print string
os.system(string)
string =str("""ncap2 -O -s 'el=""")+str(int((ranges["eta_rho"][1]-ranges["eta_rho"][0])*4000))+"""' constants_sliced.nc -o constants_sliced.nc"""
os.system(string)
print string
...
f.close()
os.system("mv h_sliced.nc grid_region.nc")
os.system("for i in *_sliced.nc; do ncks -A $i grid_region.nc;done")
os.system("rm -f *_sliced.nc")

for i in ["eta_rho", "eta_u", "eta_v", "xi_rho","xi_u","xi_v"]:
    string = """ncap2 -O -s "("""+i+"""=double("""+i+""")-"""+str(int(ranges[i][0]*4000))+""")" grid_region.nc -o grid_region.nc"""
    print string
    os.system(string)
