...
#	if foo="$(/home/mitya/testenv/bin/python -B pymframesperfile.py -o hice -v hice --var_min 0.0 --var_max 8.0 -i $file)"; then 
	if foo="$(python pytrans_read_save.py -v salt --depth_max 200 --depth_min 0 --var_min 32 --var_max 35 -is segments_fake_a20 -o salt_a20 -i $file --title='a20 restored')"; then 
	    echo "python works"
#	    read -a array <<< $foo
