...
for i in range(1,13):
    ...
    month_factor.append(1/((1000**3)/(monthrange(1970,i)[1]*24*3600.)))
print month_factor
...
river_files = (args.inf, '/home/mitya/pytools_git/a20_test.nc')
#river_files = ('/home/mitya/pytools_git/corrected_rivers.nc', '/home/mitya/models/NorROMS/Apps/Common/Origfiles/A20_rivers_openBering35.nc')
...
for hplot in [1,2]:
    rho_x, rho_e = [],[]
    ax = fig.add_subplot(1,2,hplot)
    for i,j in enumerate(river_x[hplot-1]):
        print i,j
        
        if river_transport[hplot-1][args.num-1,i]>0:
            salt[hplot-1][int(river_e[hplot-1][i]),int(river_x[hplot-1][i])] = 101.0
            rho_x.append(river_x[hplot-1][i])
            rho_e.append(river_e[hplot-1][i])
        else:
            if river_direction[hplot-1][i]==0:
                salt[hplot-1][int(river_e[hplot-1][i]),int(river_x[hplot-1][i])-1] = 101.0
                rho_x.append(river_x[hplot-1][i]-1)
                rho_e.append(river_e[hplot-1][i])
            else:
                salt[hplot-1][int(river_e[hplot-1][i]-1),int(river_x[hplot-1][i])] = 101.0
                rho_x.append(river_x[hplot-1][i])
        ...
        if abs(river_transport[hplot-1][args.num-1,i])>0:
            if river_transport[hplot-1][args.num-1,i]>0:
                atxt=ax.text(river_x[hplot-1][i],river_e[hplot-1][i],str(int(j))+\
                                 ...
                                 str(int(np.sum(river_transport[hplot-1][:,i]*month_factor))) + "\n"+\
...
ax.set_title("arctic 4km, annual discharge km**3 \n \n ")
bx = fig.add_subplot(122)
bx.set_title("arctic 20km, annual discharge km**3  \n \n")

for hplot in [1,2]:
    print river_transport[hplot-1].shape
    a = np.sum(river_transport[hplot-1], axis = 0)
    a = sign(a)*a
