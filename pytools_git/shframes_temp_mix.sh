...
archive=/global/work/mitya/rerun_mix/
cd $pathRun
echo $1 $2  ' -> echo $1 $2'
#echo $k
#echo $j
...
#	if foo="$(/home/mitya/testenv/bin/python -B pymframesperfile.py -o hice -v hice --var_min 0.0 --var_max 8.0 -i $file)"; then 
	if foo="$(python ~/pytools_git/pytrans_read_save.py -v temp --depth_max 800 --depth_min 190 --var_min -0.9 --var_max 1.5 -is ~/pytools_git/segments_aw_a4 -o mix_rerun -i $file --title='a4: C4HA,DIF4,MIX_S_TS /n tnu4=0.5d+5  ' -txt mix_rerun )"; then 
	    echo "python works"
#	    read -a array <<< $foo
